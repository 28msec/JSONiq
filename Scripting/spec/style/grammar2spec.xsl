...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- $Id: grammar2spec.xsl,v 1.9 2009/01/03 06:18:19 mdyck Exp $ -->
  ...
  <xsl:key name="non-terminals"
    match="g:*[
            self::g:production
            or self::g:xexprProduction
            or parent::g:level
			or self::g:production-for-token][not(@show='no')]"
    use="''"/>

  <xsl:key name="terminals"
    match="g:token[@inline='false' and not(@visible='false')]
            [not(normalize-space(@show)='no') and not(@show-as-production='yes')]"
    use="''"/>

  <xsl:key name="named-terminals"
    match="g:token[@inline='false' and not(@visible='false')]
            [not(normalize-space(@show)='no') and not(@show-as-production='yes')]"
    use="@name"/>

  <xsl:key name="ref" match="g:token[
                             not(@is-xml = 'yes')][not(@show='no')]
                             |g:production
                             |g:xexprProduction
  ...
  <xsl:key name="refsWithXML" match="g:token
                             |g:production
                             |g:xexprProduction
                             |g:exprProduction/g:level/*"
    use="@name"/>
...
<!-- Generate a comment that identifies as much as we can about the XSLT processor being used -->
    ...
    <spec>
      ...
      <body>
        <div1>
          <head>BNF</head>
          <!-- scrap>
            <head>XML TERMINALS</head>
            <xsl:call-template name="add-xmlspec-tokens"/>
          </scrap>

          <scrap>
            <head>XML-Names TERMINALS</head>
            <xsl:call-template name="add-xmlnames-tokens"/>
          </scrap>

          <scrap>
            <head>NON-SIGNIFICANT CONSTRUCTS</head>
            <xsl:call-template name="add-typed-tokens">
              <xsl:with-param name="type" select="'skip-token'"/>
            </xsl:call-template>
          </scrap>

          <scrap>
            <head>LITERAL TERMINALS</head>
            <xsl:call-template name="add-typed-tokens">
              <xsl:with-param name="type" select="'literal'"/>
            </xsl:call-template>
          </scrap -->
  ...
  <xsl:template name="add-nt-link">
    ...
    <xsl:param name="orig" select="@orig"/>
    <xsl:if test="not($value) or $value=''">
      <xsl:message>value missing! def: <xsl:value-of select="$def"/></xsl:message>
    </xsl:if>
<!--
<xsl:message>DEBUG: Starting template add-nt-link. </xsl:message>
<xsl:message>DEBUG: template add-nt-link ~~ $value = <xsl:value-of select="$value"/>. </xsl:message>
<xsl:message>DEBUG: template add-nt-link ~~ $spec = <xsl:value-of select="$spec"/>. </xsl:message>
<xsl:message>DEBUG: template add-nt-link ~~ $which-spec = <xsl:value-of select="local-name($which-spec)"/> and $which-spec/@if = <xsl:value-of select="$which-spec/@if"/>. </xsl:message>
<xsl:message>DEBUG: template add-nt-link ~~ $orig = <xsl:value-of select="$orig"/>. </xsl:message>
-->

    <nt>
<!-- 2008-09-23: Jim and Jonathan added "[1]" to the XPath expression in the select attribute -->
<!--   The reason was that Jim was unable to build the xquery-update-10 or xpath-full-text-10 -->
<!--   documents, because validation of update-assembled.xml would get a number of instances  -->
<!--   of: Attribute value "prod-xquery update-TypeswitchExpr" of type IDREF must be a name   -->
<!--   NOTE: It is not certain whether this is a viable permanent fix, because it might be    -->
<!--   because of a difference in the specific XSLT engine in use, or Java version, or...     -->
      <xsl:variable name="lang-id" select="(/g:grammar/g:language/@id)[1]"/>
<!--
<xsl:message>DEBUG: template add-nt-link ~~ $lang-id = <xsl:value-of select="$lang-id"/>. </xsl:message>
-->
        ...
        <xsl:choose>
          <xsl:when test="starts-with($def, 'doc-')">
            ...
            <xsl:for-each select="$sourceTree">
              <xsl:choose>
                <xsl:when test="//prodrecap[@id=$value]">
                  <xsl:choose>
                    <xsl:when test="$orig">
                  ...
                  <!-- xsl:text>doc-</xsl:text>
                  <xsl:value-of select="$lang-id"/>
                  <xsl:value-of select="substring($def, 4)"/ -->
                </xsl:when>
                <xsl:otherwise>
          ...
          <xsl:when test="starts-with($def, 'prod-') and
                          not(starts-with($def, 'prod-xpath-') or
                              starts-with($def, 'prod-xquery-') or
                              starts-with($def, 'prod-core-')) and
                          ($lang-id = 'xpath' or
                           $lang-id = 'xquery' or
                           $lang-id = 'core')">
            <xsl:text>prod-</xsl:text>
            ...
            <xsl:value-of select="substring($def, 5)"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:choose>
              <xsl:when test="$orig">
...
<!--
<xsl:message>DEBUG: template add-nt-link ~~ $def = <xsl:value-of select="$def"/>. </xsl:message>
-->
      <xsl:call-template name="add-spec-class">
  ...
  <xsl:template match="g:requiredSkip">
    <xsl:call-template name="add-nt-link">
      <xsl:with-param name="def">
        <xsl:text>prod-S</xsl:text>
      </xsl:with-param>
      <xsl:with-param name="value" select="'S'"/>
    </xsl:call-template>
  </xsl:template>

  <xsl:template match="g:requiredSkip[@show='no']">
  </xsl:template>

  <xsl:template name="wrap-with-rhs-group">
    <xsl:param name="content"/>
    <xsl:param name="which-spec"
...
<!--
  <xsl:template match="g:token">
    <xsl:param name="expand-if-token" select="true()"/>
    <xsl:param name="ref-prefix" select="'prod-'"/>
    <xsl:param name="wrapper-name" select="'rhs-group'"/>

    <xsl:call-template name="g:group">
      <xsl:with-param name="wrapper-name" select="$wrapper-name"/>
      <xsl:with-param name="conn-cur" select="$conn-top"/>
      <xsl:with-param name="expand-if-token" select="$expand-if-token"/>
...
-->
    ...
    <xsl:choose>
      <xsl:when test="@alias-for">
        <xsl:choose>
          <xsl:when test="false()">
            ...
            <xsl:apply-templates select="key('refsWithXML', @alias-for)">
              <xsl:with-param name="wrapper-name" select="$wrapper-name"/>
              <xsl:with-param name="expand-if-token" select="$expand-if-token"/>
              <xsl:with-param name="ref-prefix" select="$ref-prefix"/>
            </xsl:apply-templates>
          ...
          <xsl:otherwise>
            <xsl:call-template name="g:ref">
              ...
              <xsl:with-param name="expand-if-token" select="$expand-if-token"/>
              <xsl:with-param name="ref-prefix" select="$ref-prefix"/>
              <xsl:with-param name="tname" select="key('refsWithXML', @alias-for)/@name"/>
            </xsl:call-template>
          </xsl:otherwise>
        </xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:call-template name="g:group">
          <xsl:with-param name="wrapper-name" select="$wrapper-name"/>
          <xsl:with-param name="conn-cur" select="$conn-top"/>
          <!-- sb: force expand-if-token to true, fix bug with CommentStart. -->
          <xsl:with-param name="expand-if-token" select="true()"/>
          <xsl:with-param name="ref-prefix" select="$ref-prefix"/>
        </xsl:call-template>
        <!-- xsl:if test="@subtract-reg-expr" -->
        <!-- xsl:text> - z</xsl:text -->
  ...
  <xsl:template match="g:production-for-token">
    <xsl:param name="expand-if-token" select="true()"/>
    <xsl:param name="ref-prefix" select="'prod-'"/>
    <xsl:param name="wrapper-name" select="'rhs-group'"/>
	<xsl:variable name="name" select="@name"/>
	<xsl:apply-templates select="//g:token[@show-as-production='yes' and @name=$name]">
		<xsl:with-param name="expand-if-token" select="$expand-if-token"/>
		<xsl:with-param name="ref-prefix" select="$ref-prefix"/>
		<xsl:with-param name="wrapper-name" select="$wrapper-name"/>
	</xsl:apply-templates>
  ...
  <xsl:template match="g:production">
    ...
    <xsl:choose>
      <xsl:when test="$exp-prod and starts-with($ref-prefix, 'doc-')">
...
-->
  <xsl:template match="g:postfix">
    ...
    <xsl:call-template name="add-nt-link">
      ...
      <xsl:with-param name="value" select="$left-name"/>
    </xsl:call-template>
    <xsl:text>&nbsp;</xsl:text>
    <xsl:text>(&nbsp;</xsl:text>
    <xsl:call-template name="g:group">
  ...
  <xsl:template match="g:ref|g:xref" name="g:ref">
    ...
    <xsl:param name="conn-cur" select="$conn-none"/>
    <xsl:param name="conn-new" select="$conn-seq"/>
    <xsl:param name="which-spec" select="ancestor-or-self::*[@if][1]/@if[contains(., $spec)
              or ($spec = 'shared')]"/>
    ...
    <xsl:variable name="deref" select="key('ref', $tname)"/>
    <xsl:variable name="derefwithxml" select="key('refsWithXML', $tname)"/>

    <xsl:variable name="name">
      <xsl:choose>
		<xsl:when test="$derefwithxml/@exposition-name">
			<xsl:value-of select="$derefwithxml/@exposition-name"/>
		</xsl:when>
        <xsl:when test="$deref/@alias-for">
            <xsl:variable name="de-aliased" select="key('refsWithXML',$deref/@alias-for)"/>
			<xsl:choose>
    ...
    <xsl:if test="$show-no-shows or not(@show = 'no')">
      ...
      <xsl:for-each select="/">
        <xsl:variable name="deref2" select="key('ref',$name)"/>
        <xsl:variable name="target" select="$deref2[self::g:token and not(@inline='false')]"/>

        <xsl:choose>
          <xsl:when test="(not($target) or not($expand-if-token))
                          and not(($name='ExtensionStart')
                          or ($name='PragmaKeyword')
                          or ($name='Extension')
                          or ($name='ExtensionEnd')
                          or ($name='ExprCommentStart')
                          or ($name='ExprCommentEnd'))">
             <xsl:choose>
               <xsl:when test="false()"/>
              <!-- xsl:when test="not($prodrecaps[@ref=$name])">
                   <nt def="prod-{$name}">
                     <xsl:call-template name="add-spec-class">
                       <xsl:with-param name="which-spec" select="$which-spec"/>
                     </xsl:call-template>
              ...
              <xsl:otherwise>
							<!-- xsl:message>
							<xsl:value-of select="local-name($ref-node)"/>
							</xsl:message -->
				<xsl:variable name="zref-prefix">
					<xsl:choose>
						<xsl:when test="local-name($ref-node) = 'xref'">
							<xsl:value-of select="'doc-'"/>
						</xsl:when>
						<xsl:otherwise><xsl:value-of select="$ref-prefix"/></xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
                <xsl:call-template name="add-nt-link">
                  <xsl:with-param name="def">
                    <xsl:value-of select="$zref-prefix"/>
                    <xsl:value-of select="$name"/>
                  </xsl:with-param>
                  <xsl:with-param name="which-spec" select="$which-spec"/>
          ...
          <xsl:otherwise>
            <!-- don't turn on grouping indicators if we're in the exposition. -->
            <xsl:variable name="isLookAheadType" select="(count($target/*) &gt; 1)
                                                         and not(starts-with($ref-prefix, 'doc-'))"/>
            <xsl:choose>
              <xsl:when test="normalize-space($spec)!=normalize-space('shared')">
                <xsl:choose>
                  <xsl:when test="false() and $isLookAheadType">
                    <xsl:text>&lt;</xsl:text>
                  </xsl:when>
  ...
  <xsl:template name="add-non-terminals">
    <xsl:param name="orig"/>
    <xsl:param name="wrapper-name" select="'rhs-group'"/>
    <xsl:param name="name-prefix" select="'prod-'"/>
    <xsl:for-each select="key('non-terminals', '')">
      <xsl:variable name="name" select="@name"/>
      ...
      <xsl:apply-templates select="." mode="g:tableCells">
        ...
        <xsl:with-param name="ref-prefix" select="concat('doc-', $orig, '-')"/>
      </xsl:apply-templates>
    </xsl:for-each>
  </xsl:template>

  <xsl:template name="add-xmlspec-tokens">
    <xsl:param name="orig"/>
    <xsl:call-template name="add-external-tokens">
      <xsl:with-param name="xhref-criteria" select="'http://www.w3.org/TR/REC-xml'"/>
      <xsl:with-param name="orig" select="$orig"/>
    </xsl:call-template>
  </xsl:template>

  <xsl:template name="add-xmlnames-tokens">
    <xsl:call-template name="add-external-tokens">
      <xsl:with-param name="xhref-criteria" select="'http://www.w3.org/TR/REC-xml-names/'"/>
    </xsl:call-template>
  </xsl:template>

  <xsl:template name="add-external-tokens">
    <xsl:param name="orig"/>
    <xsl:param name="xhref-criteria"/>
    <xsl:variable name="non-terminals"
      select="g:start | g:production | g:production-for-token | g:exprProduction/g:level/*"/>
    <xsl:variable name="non-terminal-count" select="count(key('non-terminals', ''))"/>
    <xsl:variable name="terminals" select="g:token[(@is-xml = 'yes')
                                           and starts-with(@xhref,
                                           $xhref-criteria)
                                           and not(@visible='false')]"/>
    <xsl:for-each select="key('terminals', '')[@inline='false'
                          and not(@visible='false')]">
      <xsl:variable name="name" select="@name"/>
      <xsl:variable name="position" select="position()"/>
      <xsl:apply-templates select="." mode="g:tableCells">
        <xsl:with-param name="orig" select="$orig"/>
        <xsl:with-param name="name" select="$name"/>
        <!-- xsl:with-param name="position" select="$non-terminal-count+$position"/ -->
        <xsl:with-param name="expand-if-token" select="false()"/>
        <xsl:with-param name="name-prefix" select="'prod-'"/>
        <xsl:with-param name="ref-prefix">
          <!-- xsl:if test="$prodrecaps[@ref=current()/@name]"><xsl:text>doc-</xsl:text></xsl:if -->
        </xsl:with-param>
      </xsl:apply-templates>
    </xsl:for-each>
  ...
  <xsl:template name="add-typed-tokens">
    <xsl:param name="orig"/>
    <xsl:param name="type"/>
    <xsl:variable name="non-terminals"
      select="g:start | g:production | g:production-for-token | g:exprProduction/g:level/*"/>
    <xsl:variable name="non-terminal-count" select="count(key('non-terminals', ''))"/>
    <xsl:variable name="terminals" select="g:token[not(@is-xml = 'yes') and not(@visible='false')]"/>
<!--
<xsl:message>DEBUG: Starting add-typed-tokens. </xsl:message>
<xsl:message>DEBUG: $terminals = <xsl:value-of select="$terminals/@name"/>. </xsl:message>
-->
    <xsl:for-each select="key('terminals', '')[@inline='false'
                          and @type=$type and not(@visible='false')]">
      <xsl:variable name="name" select="@name"/>
      <xsl:variable name="position" select="position()"/>
      <xsl:apply-templates select="." mode="g:tableCells">
        <xsl:with-param name="orig" select="$orig"/>
...
<!--
<xsl:message>DEBUG: Exiting add-typed-tokens. </xsl:message>
-->
  </xsl:template>

  <xsl:template name="add-terminals">
    <xsl:param name="orig"/>
    <xsl:param name="suppress-typed" select="false()"/>
    <xsl:param name="name-prefix" select="'prod-'"/>
    <xsl:param name="xml-only" select="false()"/>
    <xsl:variable name="non-terminals"
      select="g:start | g:production | g:production-for-token | g:exprProduction/g:level/*"/>
    <xsl:variable name="non-terminal-count" select="count(key('non-terminals', ''))"/>
    <xsl:variable name="terminals" select="g:token[not(@visible='false')]"/>
<!--
<xsl:message>DEBUG: Starting add-terminals. </xsl:message>
<xsl:message>DEBUG: $terminals = <xsl:value-of select="$terminals/@name"/>. </xsl:message>
-->
    <xsl:for-each select="key('terminals', '')[not(@is-local-to-terminal-symbol='yes') and
		                  @inline='false'
                          and (not($suppress-typed) or not(@type))
                          and not(@visible='false')][not(@show-as-production='yes')]">
      <xsl:variable name="name" select="@name"/>
      <xsl:variable name="position" select="position()"/>
...
<!--
<xsl:message>DEBUG: Exiting addterminals. </xsl:message>
-->
  </xsl:template>

  <xsl:template name="add-local-terminals">
    <xsl:param name="orig"/>
    <xsl:param name="suppress-typed" select="false()"/>
    <xsl:param name="name-prefix" select="'prod-'"/>
    <xsl:param name="xml-only" select="false()"/>
    ...
    <xsl:variable name="non-terminal-count" select="count(key('non-terminals', ''))"/>
    <xsl:variable name="terminals" select="g:token[not(@visible='false')]"/>
    <xsl:for-each select="key('terminals', '')[(@is-local-to-terminal-symbol='yes') and @inline='false'
                          and (not($suppress-typed) or not(@type))
                          and not(@visible='false')][not(@show-as-production='yes')]">
      <xsl:variable name="name" select="@name"/>
      <xsl:variable name="position" select="position()"/>
  ...
  <xsl:template name="g:group">
    ...
    <xsl:param name="conn-cur" select="$conn-none"/>
    <xsl:param name="conn-new" select="$conn-seq"/>
	<xsl:variable name="isExpo" select="starts-with($ref-prefix, 'doc-')"/>

    <xsl:variable name="group" select="*[not(normalize-space(@show)='no')]"/>
    <xsl:choose>
  ...
  <xsl:template name="make-absolute-nt-number">
    ...
    <xsl:choose>
      ...
      <xsl:when test="key('named-terminals', $name)">
        <xsl:for-each select="/g:grammar">
          <xsl:for-each select="key('terminals', '')[not(@alias-for and not(@inline='false'))]">
            <xsl:if test="normalize-space(@name)=normalize-space($name)">
              <xsl:value-of select="position()+count(key('non-terminals', ''))"/>
            </xsl:if>
          </xsl:for-each>
        </xsl:for-each>
      </xsl:when>
  ...
  <xsl:template name="add-spec-class">
    <xsl:param name="which-spec" select="ancestor-or-self::*[@if][1]/@if[contains(., $spec) or ($spec = 'shared')]"/>
  ...
  <xsl:template match="*" mode="g:tableCells">
    <xsl:param name="orig"/>
    <xsl:param name="name-prefix" select="concat('doc-', $orig, '-')"/>
    <xsl:param name="ref-prefix" select="'prod-'"/>
    <xsl:param name="expand-if-token" select="true()"/>
    <xsl:param name="name"/>
    ...
    <xsl:param name="real-id" select="@id"/>
    <xsl:param name="real-id-node" select="."/>
    <xsl:param name="wrapper-name" select="'rhs-group'"/>

    <prod>
        <xsl:variable name="expo-name">
			<xsl:variable name="derefs" select="key('refsWithXML', $name)"/>
			<!-- xsl:message>Checking <xsl:value-of select="$name"/></xsl:message -->
			<xsl:choose>
				<xsl:when test="$derefs/@exposition-name">
					<!-- xsl:message>Putting out lhs <xsl:value-of select="$name"/> as <xsl:value-of select="$derefs/@exposition-name"/></xsl:message -->
					<xsl:value-of select="$derefs/@exposition-name"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$name"/>
				</xsl:otherwise>
      ...
      <xsl:attribute name="id">
        <xsl:if test="$real-id = '_no-id' or  starts-with($real-id, 'no-id_')">
          <!-- pull some nasty trick to handle multiply defined productions. -->
          <!-- Don wants this... -->
          <!-- Don't change the 'noid_' text... "lhs-text" template depends on it. -->
          <xsl:text>noid_</xsl:text>
          <xsl:value-of select="substring(generate-id($real-id-node), 0)"/>
          <xsl:value-of select="'.'"/>
          <!-- xsl:message>
            <xsl:value-of select="generate-id($real-id-node)"/>
          </xsl:message -->
        </xsl:if>
        <xsl:if test="$real-id = 'fakeid'">
          <xsl:text>fakeid_</xsl:text>
        </xsl:if>
      ...
      <xsl:call-template name="make-lhs">
        <xsl:with-param name="name" select="$expo-name"/>
      </xsl:call-template>
      <rhs>
        <xsl:choose>
          <xsl:when test="@xhref">
          ...
          <xsl:when test="normalize-space($spec)!=normalize-space('shared')">
            <xsl:apply-templates select=".">
              <xsl:with-param name="expand-if-token" select="$expand-if-token"/>
              <!-- The following looks strange, but is correct -->
              <xsl:with-param name="ref-prefix" select="$name-prefix"/>
            </xsl:apply-templates>
            <xsl:if test="@subtract-reg-expr">
              <xsl:text> - </xsl:text>
              <xsl:value-of select="@subtract-reg-expr"/>
      ...
      <xsl:if test="starts-with($name-prefix, 'prod-')">
  ...
  <xsl:template match="g:transition" mode="g:tableCells">
    <xsl:message terminate="yes">Internal error: matched g:transition</xsl:message>
  </xsl:template>

  <xsl:template name="show-dt">
    <xsl:param name="type"/>
<!-- Jim 20090101: For the joint spec, the original way of computing the value of ref-prefix -->
<!--                 produced the wrong answer; I replaced that declaration with a series -->
<!--                 of declarations (fn, grammar, ref-prefix-middle-part, then ref-prefix) -->
<!--                 that computes the prefix in the same manner it's done throughout the -->
<!--                 stylesheets that are used to build the "assembled" source specs. -->
<!--    <xsl:variable name="ref-prefix" select="concat('prod-',$spec,'-')"/> -->
    <xsl:variable name="fn"><xsl:call-template name="get-gfn"/></xsl:variable>
    <xsl:variable name="grammar" select="document($fn,.)"/>
    <xsl:variable name="ref-prefix-middle-part">
      <xsl:value-of select="$grammar/g:grammar/g:language/@id"/>
    </xsl:variable>
    <xsl:variable name="ref-prefix" select="concat('prod-',$ref-prefix-middle-part,'-')"/>
...
<xsl:message>DEBUG: template show-dt ~~ $ref-prefix = <xsl:value-of select="$ref-prefix"/>. </xsl:message>
-->
    <xsl:for-each select="document($tokens-file,.)/token-list/token[@type = $type]">
      <xsl:choose>
        <xsl:when test="@expo-name and string-length(.) = 0">
            ...
            <xsl:with-param name="value" select="@expo-name"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
          ...
          <def>
            ...
            <p>
              <table border = "1" summary="Transition table">
                ...
                <tbody>
                  <xsl:for-each select="g:transition[g:tref[key('refsWithXML', @name)]]">
                    <tr>
                      <td>
                        <xsl:for-each select="g:tref[not(@show='no') and key('refsWithXML', @name)]">
                          <xsl:choose>
        ...
        <xsl:choose>
          <xsl:when test="$ref">
            <xsl:choose>
              ...
              <xsl:when test="key('refsWithXML', $string)">
                 <xsl:for-each select="$ref">
                  <xsl:call-template name="g:ref">
                    <xsl:with-param name="wrapper-name" select="$wrapper-name"/>
                    <xsl:with-param name="ref-prefix" select="'prod-'"/>
                    <xsl:with-param name="conn-cur" select="$conn-top"/>
