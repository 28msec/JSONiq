...
<!-- Generate a comment that identifies as much as we can about the XSLT processor being used -->
  ...
  <xsl:template name="show-dt">
    ...
    <xsl:param name="t-list" select="/g:grammar//*[not(ancestor-or-self::*[@show='no']) and (
                                      self::g:string[not(ancestor::g:token)] or
									  self::g:string[ancestor::g:token[/g:grammar//g:ref/@name=@name
									     and (@inline='true') and not(@special='yes')]] or
									  self::g:token[not(@alias-for) and /g:grammar//g:ref/@name=@name
									     and not(@special='yes') and not(@inline='true')
									        and not(@show-as-production='yes')]
									)]"/>
    ...
    <xsl:for-each select="$t-list">
		...
		<xsl:variable name="isPattern" select="self::g:token[g:ref[position()=1] or g:optional or g:choice or g:sequence or g:charClass or g:oneOrMore or g:zeroOrMore]"/>

		<xsl:choose>
			<xsl:when test="@delimiter-type='hide'">
			</xsl:when>
			<xsl:when test="@show='false' or @visible='no'
						  or @special='yes' or @is-macro='yes' or @visible='false'">
			</xsl:when>
			<xsl:when test="self::g:token[count(g:ref) = count(*) and count(g:ref) = 1 and not(@inline='false')]">
			</xsl:when>
			<xsl:otherwise>
